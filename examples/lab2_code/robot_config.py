...
# Defining my own robot for Lab 1
def newRobot():
    
    # Create one rectangle object
    body = add_object([0.15, 0.25], [0.03, 0.1])
    # Create front rectangle object
    front = add_object([0.2, 0.35], [0.1, 0.03])
    
    # Add joint between the two rectangles
    joint_stiffness = 50
    add_spring(body, front, [0, 0.08], [-0.08, 0], -1, joint_stiffness)
    
    # Add a spring in between the two rectangles
    rest_length = 0.15
    stiffness = 10
    act = 0.08
    add_spring(body, front, [0, -0.08], [0.08, 0], rest_length, stiffness, actuation=act)
    
    # Add another rectangle
    second = add_object([0.25, 0.45], [0.03, 0.1])
    
    # Add joint between the two rectangles
    add_spring(front, second, [0.08, 0], [0, -0.08], -1, joint_stiffness)
    
    # Spring between the second and first rectangle
    add_spring(front, second, [-0.08, 0], [0, 0.08], rest_length, stiffness, actuation=act)
    
    # Add another rectangle
    third = add_object([0.3, 0.55], [0.1, 0.03])
    
    # Add joint between the two rectangles
    add_spring(second, third, [0, 0.08], [-0.08, 0], -1, joint_stiffness)
    
    # Spring between the second and third rectangle
    add_spring(second, third, [0, -0.08], [0.08, 0], rest_length, stiffness, actuation=act)
    
    # Add a 4th rectangle
    # fourth = add_object([0.35, 0.65], [0.03, 0.1])
    # add_spring(third, fourth, [0.08, 0], [0, -0.08], -1, joint_stiffness)
    # add_spring(third, fourth, [-0.08, 0], [0, 0.08], rest_length, stiffness, actuation=act)
    
    # # Adding this rectangle causes a "cannot convert float NaN to integer" error
    # # Add a 5th rectangle
    # fifth = add_object([0.4, 0.75], [0.1, 0.03])
    # add_spring(fourth, fifth, [0, 0.08], [-0.08, 0], -1, joint_stiffness)
    # add_spring(fourth, fifth, [0, -0.08], [0.08, 0], rest_length, stiffness, actuation=act)
    
    return objects, springs, body


robots = [robotA, robotB, robotLeg, newRobot]
